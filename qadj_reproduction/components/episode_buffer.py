    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            if isinstance(v, th.Tensor):
                ...
                # Avoid the slow path and extra temporary allocations when
                # input is a Python list of NumPy arrays.
...
class ReplayBuffer(EpisodeBatch):
    ...
    def save_state(self, path):
        state = {
            ...
            "transition_data": {k: v.detach().cpu() for k, v in self.data.transition_data.items()},
            "episode_data": {k: v.detach().cpu() for k, v in self.data.episode_data.items()},
        }
        th.save(state, path)
    ...
    def load_state(self, path):
        ...
        self.episodes_in_buffer = state["episodes_in_buffer"]
        for k, v in state["transition_data"].items():
            self.data.transition_data[k] = v.to(self.device)
        for k, v in state["episode_data"].items():
