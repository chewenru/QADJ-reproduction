...
class WeightedQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer != "qmix":
            raise ValueError("WeightedQLearner currently supports mixer='qmix' only.")
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        weights = th.ones_like(td_error)
        negative_mask = td_error < 0
        weights[negative_mask] = getattr(self.args, "wqmix_weight", 0.1)

        mask = mask.expand_as(td_error)
        weighted_td_error = weights * (td_error ** 2) * mask
        loss = weighted_td_error.sum() / mask.sum()
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("wqmix_downweighted_frac", (negative_mask.float() * mask).sum().item() / mask_elems, t_env)
